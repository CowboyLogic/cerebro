...
describe('Instruction install → VS Code (workspace scope, append)', () => {
  it('creates copilot-instructions.md when none exists', async () => {
    ...
    expect(result.success).toBe(true);
    const { fs } = await import('memfs');
    const writtenPath = path.join('/proj', '.github', 'copilot-instructions.md');
    expect(fs.existsSync(writtenPath)).toBe(true);
  });
  ...
  it('appends to existing copilot-instructions.md', async () => {
    ...
    // Pre-create existing file at the correct location: <workspaceRoot>/.github
    fs.mkdirSync(path.join('/proj', '.github'), { recursive: true });
    fs.writeFileSync(path.join('/proj', '.github', 'copilot-instructions.md'), '# Existing');
    ...
    const filePath = path.join('/proj', '.github', 'copilot-instructions.md');
