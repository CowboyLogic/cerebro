...
describe('VSCodeInstaller.installAsInstructions — install path correctness', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockReadFileSync.mockImplementation(() => { throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' }); });
  });

  it('workspace scope writes to <workspaceRoot>/.github, not <installDir>/.github', async () => {
    const comp = makeComponent({ type: 'instruction' });
    comp.files[0].content = '# Instructions';
    const result = await installer.install(
      makeOpts({ component: comp, scope: 'workspace', workspaceRoot: '/proj' })
    );

    expect(result.success).toBe(true);
    const writePath = mockWriteFileSync.mock.calls[0][0] as string;
    expect(writePath).toBe(path.join('/proj', '.github', 'copilot-instructions.md'));
    // Must NOT contain .vscode in the path
    expect(writePath.split(path.sep)).not.toContain('.vscode');
  });

  it('user scope writes to <installDir>/.github/copilot-instructions.md', async () => {
    vi.spyOn(installer, 'getInstallDir').mockReturnValue('/fake/vscode/user');
    const comp = makeComponent({ type: 'instruction' });
    comp.files[0].content = '# Instructions';
    const result = await installer.install(
      makeOpts({ component: comp, scope: 'user', workspaceRoot: '/proj' })
    );

    expect(result.success).toBe(true);
    const writePath = mockWriteFileSync.mock.calls[0][0] as string;
    expect(writePath).toBe(path.join('/fake/vscode/user', '.github', 'copilot-instructions.md'));
  });
});

describe('VSCodeInstaller.installAsInstructions — assertConfined security', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockReadFileSync.mockImplementation(() => { throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' }); });
  });

  it('calls assertConfined before writeFileSync', async () => {
    const assertSpy = vi.spyOn(BaseInstaller as any, 'assertConfined');
    const comp = makeComponent({ type: 'instruction' });
    comp.files[0].content = '# Instructions';
    await installer.install(makeOpts({ component: comp, scope: 'workspace', workspaceRoot: '/proj' }));
    ...
    expect(assertOrder).toBeLessThan(writeOrder);
    assertSpy.mockRestore();
  });

  it('does not write and captures error when assertConfined throws', async () => {
    vi.spyOn(BaseInstaller as any, 'assertConfined').mockImplementation(() => {
      throw new Error('Security: path escapes install directory');
    });
    const comp = makeComponent({ type: 'instruction' });
    comp.files[0].content = '# Instructions';
    const result = await installer.install(
