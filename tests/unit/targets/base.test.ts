...
const { mockWriteFileSync, mockMkdirSync, mockEnsureDir, mockLoggerDebug } = vi.hoisted(() => ({
...
describe('BaseInstaller.install', () => {
  ...
  describe('path confinement (assertConfined)', () => {
    ...
    it('logs assertConfined ok when a confined path is accepted', async () => {
      const comp = makeComponent();
      comp.files[0] = { path: 'subdir/file.md', name: 'file.md', content: '# ok' };
      const safeInstaller = new class extends BaseInstaller {
        get name() { return 'Safe'; }
      ...
      await safeInstaller.install(makeOpts({ component: comp }));
      expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringContaining('assertConfined ok'));
    });
  });
});
