...
describe('installComponent — invalid target', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns { success: false, errors } instead of throwing when getInstaller throws', async () => {
    const { getFileContents } = await import('../../../src/core/github.js');
    const { getInstaller } = await import('../../../src/targets/index.js');
    vi.mocked(getFileContents).mockResolvedValue([]);
    vi.mocked(getInstaller).mockImplementation(() => {
      throw new Error('Unknown install target: "bad". Valid targets: claude-code, opencode, vscode, copilot');
    });

    const { installComponent } = await import('../../../src/core/installer.js');
    const comp = makeComponent();
    const result = await installComponent(comp, 'bad' as any, 'user');

    expect(result.success).toBe(false);
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]).toMatch(/Unknown install target/);
